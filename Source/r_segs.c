...
static fixed_t R_ScaleFromGlobalAngleWiggle (angle_t visangle)
{
...
void R_StoreWallRange(const int start, const int stop)
{
  ...
  ds_p->scale1 = rw_scale =
    R_ScaleFromGlobalAngleWiggle (viewangle + xtoviewangle[start]);

  if (stop > start)
    {
      ds_p->scale2 = R_ScaleFromGlobalAngleWiggle (viewangle + xtoviewangle[stop]);
